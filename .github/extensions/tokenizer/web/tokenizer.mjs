// Ported 1:1 from src/lib/tokenizer.ts to dependency-free ESM so the canvas
// iframe and the Node-side extension share a single tokenization source of
// truth. Token boundaries MUST match src/lib/tokenizer.ts (spec FR-002).

const encoder = new TextEncoder();

/**
 * @typedef {"word"|"number"|"whitespace"|"newline"|"punctuation"|"operator"|"emoji"|"symbol"|"other"} TokenCategory
 */
...
export function summarizeTokens(text, tokens = tokenize(text), model) {
  const summary = {
...
export function estimateContextPercentage(tokenCount, contextWindow) {
  if (contextWindow <= 0) {
    return 0;
  }
...
export function estimateInputCost(tokenCount, inputCostPerMillionTokens) {
...
function matchNextToken(text, position) {
  ...
  const fallback =
    codePoint === undefined ? text[position] : String.fromCodePoint(codePoint);
...
function tokenId(text, category) {
  let hash = 0x811c9dc5;
