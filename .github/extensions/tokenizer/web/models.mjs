// Ported 1:1 from src/lib/copilotModels.ts to dependency-free ESM. Pricing and
// context windows are kept in sync with the web app so the canvas cost
// comparison (spec FR-005/FR-006) matches src/lib/copilotModels.ts.

export const AI_CREDIT_USD_VALUE = 0.01;
export const AUTO_MODEL_DISCOUNT = 0.1;

export const COPILOT_MODEL_FAMILIES = [
...
export const COPILOT_AUTO_MODEL = {
  ...AUTO_MODEL_BASE,
  id: "auto",
  name: "Auto",
  provider: "Copilot",
...
export const COPILOT_MODEL_OPTIONS = [COPILOT_AUTO_MODEL, ...COPILOT_MODELS];

export function modelsForFamily(familyId) {
...
export function usdToAiCredits(usd) {
  return usd / AI_CREDIT_USD_VALUE;
}

export function aiCreditsToUsd(aiCredits) {
  return aiCredits * AI_CREDIT_USD_VALUE;
}

export function inputAiCreditsPerMillionTokens(model) {
  return usdToAiCredits(model.pricing.inputPerMillionTokensUsd);
}

export function estimateInputUsd(tokenCount, model) {
  return (tokenCount / 1_000_000) * model.pricing.inputPerMillionTokensUsd;
}

export function estimateInputAiCredits(tokenCount, model) {
  return usdToAiCredits(estimateInputUsd(tokenCount, model));
}

export function estimateOutputUsd(tokenCount, model) {
  return (tokenCount / 1_000_000) * model.pricing.outputPerMillionTokensUsd;
}

export function estimateOutputAiCredits(tokenCount, model) {
